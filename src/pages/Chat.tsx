...
const Tab3: React.FC = () => {
  ...
  // toggle "autoscroll to newest" for the ALL / a TG tab (default ON). No view refresh
  ...
  const toggleTabAutoscroll = async (val: string) => {
    ...
    if (val === "DM") {
      ...
      const tg = parseInt(val);
      if (isNaN(tg)) return;
      const set = parseTGset(s.autoscrollTGs); // membership = autoscroll OFF
      if (set.has(tg)) set.delete(tg); else set.add(tg);
      const csv = Array.from(set).join(",");
      AppPrefsStore.update(x => { x.autoscrollTGs = csv; });
      await DatabaseService.setPref('autoscrollTGs', csv);
    }
  };
  ...
  const tabMenuButtons = (): any[] => {
    ...
    return [
      ...
      { text: (autoscrollEnabledFor(val) ? "✓ " : "") + "Autoscroll to newest", handler: () => { toggleTabAutoscroll(val); } },
  ...
  // clear any pending divider-hide timer when Chat unmounts
  useEffect(() => () => { if (dividerHoldTimer.current) clearTimeout(dividerHoldTimer.current); }, []);
  ...
  useEffect(() => {
    ...
    if (notifyLevel > 0 && isAppActive && thisPageActive.current && msgType === segmentFilter) {
      ...
      if (idleMs >= AWAY_IDLE_MS) playBeep();
      notifyLevel = 0;
    }
    if (notifyLevel > 0) {
    ...
    <IonPage>
      ...
      <IonContent className="ion-padding" ref={contentRef} scrollEvents={true} onIonScroll={onContentScroll} onTouchStart={onContentTouchStart}>
        ...
        <IonAlert
          ...
          message="Node disconnected! Auto-Reconnect is disabled currently. If your node has a BLE PIN set, enter it with the key icon next to the device name."
          buttons={[
            {
              text: "OK"
            },
